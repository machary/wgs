...
Animator_Ro.prototype = {
    ...
    addItem: function (symbols, points, rudals, unit, $slider, $durasi, $value) {
        ...
        while (random_color == '#ffffff' || random_color == '#ff0000');
        ...
        $slider.slider({
            ...
            change: function (e, ui) {
                ...
                self.resetLeft(ui.value, self, $slider);
            }
        });


        this.rewidthHandler(this, $slider);
        this.resetLeft($value, this, $slider);
        if( h_end > 1) {
        ...
        for (i = 0, l = this.data.length; i < l; i++) {
            ...
            if (item.route.length > 1) {
                ...
                for (j=0; j<item.rudals.length; j++) {
                    item.rudalState.push(delayStates1.concat(item.formation.generateRudalStates(item.route, this.timeConvertion, item.unit, item.rudals[j])).concat(delayStates2));
                    maxPoint = (maxPoint < item.rudalState[j].length) ? item.rudalState[j].length : maxPoint;
                }
            }
            this.maxState = maxPoint - 1;
            this.rewidthHandler( this, item.$slider);
        }
...
//        }
    ...
    actualTime: function (ii) {
        ...
        var hours = (parseInt(ii / 10 * this.timeConvertion)) + this.minStartTime;
    ...
    resetLeft: function (value, object, slider) {
        var el = slider.find('a');
        var gerak = (value) / (object.maxStartTime - object.minStartTime) * 100;
    ...
    rewidthHandler: function ( object, slider) {
        var sliderRange = slider.slider('option', 'max');
        var defaultWidth = (slider.width()) / object.rangeTime;
        var widthHandler = (object.rangeTime - sliderRange) * defaultWidth;
