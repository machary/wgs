...
$(function(){
    ...
    pergerakan = new OpenLayers.Layer.Vector('Pergerakan');
    ...
    map.addLayers([
        intel_darat_poly, intel_laut_poly, intel_udara_poly,
        intel_darat_point, intel_laut_point, intel_udara_point,
        pergerakan
    ]);

    map.setLayerIndex(pergerakan, 0); //set the image overlay to the bottom
...
//    var root3 = document.getElementById('info-cv');
    for(var i in places)
    {
        popupContentHTML = '<table style="background: #fff;" class="popup sendiri">'
            +'<tr>'
            +'<th colspan="3">'
            + places[i]['negara']
            +'</th>'
...
//        root3.appendChild(cvE);
	    ...
	    {
		    case 1:
			    var iconUrl = baseUrl + '/images/icons/blue-ops-AU.png';
			    break;
		    case 2:
			    var iconUrl = baseUrl + '/images/icons/blue-ops-AL.png';
			    break;
		    case 3:
			    var iconUrl = baseUrl + '/images/icons/blue-ops-AD.png';
			    break;
	    }
    ...
    map.events.register('changelayer', null, function(evt){
            ...
            switch( true ){
                case (evt.layer.name == 'Point Udara' && intel_udara_point.visibility == true) :
                        parseData(1, 'point', '/ops/situasi/markers', 1);
                    break;

                case (evt.layer.name == 'Point Laut' && intel_laut_point.visibility == true) :
                        parseData(2, 'point', '/ops/situasi/markers', 2);
                    break;

                case (evt.layer.name == 'Point Darat' && intel_darat_point.visibility == true) :
                        parseData(3, 'point', '/ops/situasi/markers', 3);
                    break;

                case (evt.layer.name == 'Area Udara' && intel_udara_poly.visibility == true) :
                        parseData(1, 'polygon', '/ops/situasi/polygon', 1);
                    break;

                case (evt.layer.name == 'Area Laut' && intel_laut_poly.visibility == true) :
                        parseData(2, 'polygon', '/ops/situasi/polygon', 2);
                    break;

                case (evt.layer.name == 'Area Darat' && intel_darat_poly.visibility == true) :
                        parseData(3, 'polygon', '/ops/situasi/polygon', 3);
                    break;

                case (evt.layer.name == 'Pergerakan' && pergerakan.visibility == true) :
                        parseData(0, 'pergerakan', '/ops/situasi/movement', 0);
                    break;
            }
    });
}

function parseData(layer_id, jenis, url, matra)
{
    $.post(baseUrl + url,
        {
            "layer_id": layer_id
