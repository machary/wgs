<?php
class Peta_Model_Intelijen Extends Zend_Db_Table_Abstract
{
    public function datatablesJSONApi($areaLevel, $areaID, $sEcho, $limit = 0, $offset = 0, $sortColumn = 0, $order = 'ASC',
                                      $filter = '', $search = '', $view)
    {
        $tableGunCategory = new Peta_Model_DbTable_Intelijen();
    ...
    public function datatableseditJSONApi($areaLevel, $areaID, $sEcho, $limit = 0, $offset = 0, $sortColumn = 0, $order = 'ASC',
                                      $filter = '', $search = '', $view)
    {
        $tableGunCategory = new Peta_Model_DbTable_Intelijen();

        $data = $tableGunCategory->getalldataintelijen($limit, $offset, $sortColumn, $order, $filter, $search, $count = false);

        $queryrowsCount = $tableGunCategory->getalldataintelijen($limit, $offset, $sortColumn, $order, $filter, $search, $count = true);
        ...
        if($queryrowsCount > 0)
        {
            ...
            foreach($data as $dat)
            {
                ...
                $temp['matra']      = $dat['matra'];
                $temp['action']     = '<a href="'.$hUrl->url(array('action'=>'editintelijen', 'id'=>$dat['id'], 'matra'=>$dat['matra'])).'">Ubah - </a>';
                $temp['action']     .= '<a onclick="return confirm(\'Anda yakin ?\')" href="'.$hUrl->url(array('action'=>'deleteintelijen', 'id'=>$dat['id'])).'">Hapus</a>';
