 ...
 * @auhtor : Erlan Dwinov
 * */
class Peta_IntelijenController extends App_CrudController
{
    ...
    public function adddaratAction()
    {
        ...
        $form = new Peta_Form_Intelijen_DaratForm();
    ...
    public function dataapiAction()
    {
        ...
        $model = new Peta_Model_Intelijen();
        // Param
    ...
    public function editinformasiAction()
    {
        ...
        $userForm = new Peta_Form_Intelijen_Editcog();
        $userModel = new Peta_Model_DbTable_Intelijen();
        ...
        if($request->isPost() AND $userForm->isValid($this->_request->getPost())){
            ...
            $this->_redirect('peta/intelijen/cog');
        }else{
            if($result != null)
            {
    ...
    public function dataapieditAction()
    {
        ...
        //get rows
        $jsonString = $model->datatableseditJSONApi($areaLevel, $areaID,
            $sEcho, $limit, $offset,
            $sortColumn, $order,
            $filter, $sSearch,
            $this->view);
    ...
    public function editintelijenAction()
    {
        $request = $this->getRequest();
        $id = $this->_getParam('id');
        ...
        if($matra == 'Laut')
        {
            $form = new Peta_Form_Intelijen_LautForm();
        }
        else if($matra == 'Udara')
        {
            $form = new Peta_Form_Intelijen_UdaraForm();
        }
        else if($matra == 'Darat')
        {
            $form = new Peta_Form_Intelijen_DaratForm();
        ...
        if($request->isPost() AND $form->isValid($this->_request->getPost())){
	        ...
	        $identity = Zend_Auth::getInstance()->getStorage()->read();
	        $user = $model->getSkenAndLog($identity->id);
	        $model->updateIntelijen($_POST, $idTaktis, $jumKekuatan, $id, $matra, $keteranganArr);
	        $this->_redirect('peta/intelijen');
            ...
            if( !empty($result) ){
	            ...
	            for($g=0;$g<=$count;$g++)
	            {
		            ...
		            array_push($unsurArr, $datSimTak['nama']);
    ...
    public function deleteintelijenAction()
    {
        $id = $this->getRequest()->getParam('id');

        $model = new Peta_Model_DbTable_Intelijen();

        $model->deleteintelijen($id);
        $this->_redirect('peta/intelijen/edit');
    }

    public function pergerakanAction()
    {
        $request = $this->getRequest();
        $form = new Peta_Form_Intelijen_Pergerakan();
        $model = new Peta_Model_DbTable_Intelijen();
