...
//            ->joinLeft(array('langkah' => 'master.langkah'), 'langkah."nama_prosrenmil" like pros."nama_prosrenmil" AND langkah."nomor_langkah" = mprod."id_langkah"', array())
	...
	{
        /**
         * 1. pelaku hanya bisa melihat produk staff sesama kogas dalam 1 team
         * 2. wasdal dapat melihat selurih produk staff
         * 3. penilai hanya dapat melihat produk staff dari kogas yg dinilai nya saja
         */
		$identity = Zend_Auth::getInstance()->getStorage()->read();
        ...
        $query = $table->select()->setIntegrityCheck(false)
            ...
            ->joinLeft(array( 'login' => 'user.logins' ), 'login."id" = prod."id_user"', array() )
            ->joinLeft(array( 'role' => 'user.roles' ), 'role."id" = login."role_id"', array() )
            ->joinLeft(array( 'jabatan' => 'master.M_JABATAN' ), 'jabatan."id_jabatan" = role."kogas"', array() )

            ->joinLeft(array( 'mprod' => 'master.M_PRODUCT_STAFF' ), 'mprod."id" = prod."idm_product_staff"', array() )
            ->joinLeft(array( 'langkah' => 'master.langkah' ), 'langkah."id_langkah" = mprod."id_langkah"', array() )
            ->joinLeft(array( 'tim' => 'user.Team' ), 'tim."id" = login."id_team"', array() )
            ->joinLeft(array( 'skenario' => 'latihan.skenario' ), 'skenario."id" = tim."kode_skenario"', array() )
        ;

        switch($identity->role_id)
        {
            case 9:

                break;
            case 3:
                $tableTambahan = new Zend_Db_Table( 'user.penilai' );
                $query2 = $table->select()->setIntegrityCheck( false )
                                ->from( 'user.penilai' )
                                ->where( 'id_login = ?', $identity->id )
                ;
                $result = $tableTambahan->fetchRow( $query2 )->toArray();

                $query->where( 'jabatan."id_jabatan" = ' . $result['id_jabatan'] );
                break;
            default:
                $tableTambahan = new Zend_Db_Table( 'user.logins' );
                $query2 = $table->select()->setIntegrityCheck( false )
                    ->from( array('login' => 'user.logins' ), array( 'id_jabatan' => 'jabatan.id_jabatan' ))
                    ->joinLeft( array( 'role' => 'user.roles' ), 'role."id" = login."role_id"', array() )
                    ->joinLeft( array( 'jabatan' => 'master.M_JABATAN' ), 'jabatan."id_jabatan" = role."kogas"', array() )
                    ->where( 'login."id" = ?', $identity->id )
                ;
                $result = $tableTambahan->fetchRow( $query2 )->toArray();

                $query->where( 'jabatan."id_jabatan" = ' . $result['id_jabatan'] );
                $query->where( 'tim."id" = ' . $identity->id_team );
                break;
        }
