...
class Cms_Model_ProdukStaff extends Cms_Model_Crud
{
    ...
    public function form()
    {
        ...
        if ( empty($dataMod[0]['prosrenmil_id']) )
        {
            $f->setMethod('post')
                ...
                'label' => 'File Produk Staff ( doc, docx, pdf )',
                'required' => true,
                'destination' => './pstaff/', // hardcoded lokasi upload
                'validators' => array(
                    array('Count', false, 1),
                    array('Extension', false, array( 'doc', 'docx', 'pdf')),
                ),
            ))
        ...
        else
        {
            $f->setMethod('post')
                ...
                'label' => 'Nama Produk Staff',
                'multiple' => false,
                'multiOptions' => $this->_mprodstaffOptions($dataMod[0]['prosrenmil_id']),
                'validators' => array(
                ),
            ))
                ->addElement('text', 'text_product_staff', array(
                'label' => 'Nama Produk Staff (Kosongkan jika memilih pilihan diatas)',
                'class' => 'large',
                'validators' => array(
                ),
            ))
    ...
    protected function _mprodstaffOptions($idprosrenmil = null)
    {
        $db = $this->table()->getAdapter();
        $select = $db->select()
            ->from(array('mstaff' => 'master.M_PRODUCT_STAFF'),array('id','nama_product_staff'))
            ->joinLeft(array('mlangkah'=>'master.M_LANGKAH_PROSRENMIL'), 'mstaff.id_langkah = mlangkah.id_langkah')
            ->order('nama_product_staff ASC')
        ;

        if($idprosrenmil) {
            $select->where('mlangkah.id_prosrenmil = ?', $idprosrenmil);
        }

        $raw = $db->fetchAll($select);
        $result = array('' => 'Pilih Nama Produk Staff');
        foreach ($raw as $row) {
            $result[$row['id']] = $row['nama_product_staff'] . ' [ ' . $row['nama_langkah'] . ' ]';
        }
        return $result;
    }
     ...
     * @override tambahkan path pada file
     */
    public function setFromPost($post, $file, $Ofile)
    {
        $this->_values = $post;
        $this->_values['filepath'] = './pstaff/' . $file;
        $this->_values['original_filepath'] = './pstaff/' . $Ofile;
    }
