...
class Cms_ProdukStaffController extends App_CrudController
{
	...
	public function testAction(){
		...
		//echo shell_exec('/home/seskoal/doc/convert.sh');
		//echo 111;
		$urlconverter = 'http://10.1.1.103:81/?file=percobaan-file-5023500d41bc6.docx';
	...
	public function addAction()
	{
		...
		if ($this->_request->isPost()) {
			...
			if ($form->isValid($this->_request->getPost())) {
                ...
                if($dataMod[0]['id'] == 1 || $dataMod[0]['id'] == 2)
                {
                    $model->addMProdStaff($form->idm_product_staff->getValue());
                    $newProdStaff = $model->getMProdStaff();

                    $idmProdStaffVal = $newProdStaff['id'];
                    $formPost = $this->_request->getPost();
                    $formPost['idm_product_staff'] = $newProdStaff['id'];
                ...
                else
                {
                    ...
                    $formPost = $this->_request->getPost();
					//the following will rename the file (I'm setting the upload dir in the form)
					$namaProductStaff = $idmProdStaffVal;
				}
				...
				$uniqID = uniqid();
				$newFilename = str_replace(' ', '', $namaProductStaff) . $uniqID . '.' . $originalFilename['extension'];
				$newFilenameWF = str_replace(' ', '', $namaProductStaff) . $uniqID . '.pdf';
				...
				$crud->setFromPost($formPost, $newFilenameWF );
	...
	public function ambilAction() 
	{
		...
		if ($ref->exists()) {
		echo $ref->get('filepath') .'<br/>';
		echo 'DEVELOPERTEST5028a75f62699.pdf';
		//echo UPLOAD_PATH;
		//header("Location:".$ref->get('filepath'));
		} else {
			$this->_redirector->gotoSimple('index');
		}		
	...
	public function downloadAction()
	{
		$this->getHelper('viewRenderer')->setNoRender(true);
       	$this->getHelper('layout')->disableLayout();
		$ref = new Cms_Model_ProdukStaff(null, $this->_request->getParam('id'));
		// echo $ref->get('filepath');
		// exit;
		if ($ref->exists()) {
			$this->_redirector->gotoUrl($ref->get('filepath'));
			$this->getResponse()->setHeader('Content-type', 'application/pdf')
								->setHeader('X-Sendfile', $ref->get('filepath'))
								->sendResponse();
	...
	public function dcAction()
	{
		...
		header('Content-Disposition: attachment; filename="dopdf-7.zip"');
		readfile('http://10.1.1.103/wgs/public/upload/dopdf-7.zip');	
